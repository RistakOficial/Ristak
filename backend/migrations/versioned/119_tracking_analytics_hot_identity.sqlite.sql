...
ALTER TABLE tracking_analytics_projection_state
  ADD COLUMN range_status TEXT NOT NULL DEFAULT 'pending'
    CHECK (range_status IN ('pending', 'compiling_ranges', 'ready'));
ALTER TABLE tracking_analytics_projection_state
  ADD COLUMN range_compile_cursor TEXT;
ALTER TABLE tracking_analytics_projection_state
  ADD COLUMN range_backfill_complete INTEGER NOT NULL DEFAULT 0
    CHECK (range_backfill_complete IN (0, 1));
