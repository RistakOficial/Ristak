...
WITH video_blocks AS (
  SELECT
    ...
    block.content,
    block.created_at,
    CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END AS settings_json,
    COALESCE(
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.mediaAssetId') AS TEXT)), ''),
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.media_asset_id') AS TEXT)), '')
    ) AS explicit_asset_id,
    COALESCE(
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.mediaUrl') AS TEXT)), ''),
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.media_url') AS TEXT)), ''),
      NULLIF(TRIM(COALESCE(block.content, '')), '')
    ) AS media_url,
    COALESCE(
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.pageId') AS TEXT)), ''),
      NULLIF(TRIM(CAST(json_extract(CASE WHEN json_valid(block.settings_json) THEN block.settings_json ELSE '{}' END, '$.page_id') AS TEXT)), ''),
      'page-1'
    ) AS public_page_id
...
INSERT OR IGNORE INTO site_video_placements (
...
SELECT
  'site_video_placement_' || lower(hex(randomblob(16))),
  resolved.site_id,
  resolved.id,
