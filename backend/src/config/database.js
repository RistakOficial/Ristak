...
async function initTablesUnlocked() {
  try {
    ...
    // Entrega fiscal durable: cada PDF/XML conserva su propia idempotencia y
    // reintento. El timbrado no se revierte si un canal de salida está caído.
    await db.run(`
    ...
    // Un reemplazo de video puede conservar el historial sin reescribir el
    // ledger inmutable. Esta proyección resuelve cada asset histórico hacia el
    // asset canónico visible y la bitácora conserva la decisión explícita.
    await db.run(`
