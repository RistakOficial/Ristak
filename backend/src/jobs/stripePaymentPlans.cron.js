...
async function runStripePaymentPlans(source = 'interval') {
  ...
  try {
    await trackDeployDrainWork('cron:stripe-payment-plans', async () => {
      // (CRON-009) Lock distribuido: si hay varias instancias, solo una cobra parcialidades
      // en este tick (defensivo; con 1 instancia es inofensivo). La idempotencia de Stripe
      // ya protege contra doble cargo, esto evita además trabajo/efectos duplicados.
