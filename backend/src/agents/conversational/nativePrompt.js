...
const CAPABILITY_INSTRUCTIONS = {
  schedule_appointment: ({ summary = '', missingConfiguration = [], config = {} } = {}) => {
    ...
    return [
      ...
      'La política de empalme se toma directamente del calendario al consultar y al guardar. Nunca la inventes ni la cambies desde la conversación.',
      ...
      'book_appointment, request_human_booking y reschedule_appointment sólo actúan sobre la última oferta estructurada creada por offer_appointment_slot. No les mandes fecha, hora ni evidencia copiada: el servidor recupera esos hechos y vuelve a comprobar el calendario justo antes de guardar. La oferta no vence por tiempo, pero sí se reemplaza si la persona pide otras opciones y se rechaza si el horario dejó de estar disponible.',
...
function dataRequirementsSection(config = {}, capabilityManifest = [], { followUpMode = false } = {}) {
  ...
  if (fields.length && requirements.updateContact?.enabled) {
    lines.push('Cuando quien escribe confirme un dato suyo, usa save_contact_data. Esa herramienta actualiza únicamente al contacto solicitante del hilo: nunca guardes ahí el nombre, teléfono o correo del titular distinto ni de un invitado. El servidor puede llenar vacíos o reemplazar nombres provisionales, incluidos teléfonos, emojis, símbolos y etiquetas genéricas del canal; un dato válido distinto se conserva como alternativo para revisión y nunca se sobrescribe sólo porque tú envíes un booleano de confirmación.')
