const CAPABILITY_INSTRUCTIONS = {
  schedule_appointment: ({ summary = '', missingConfiguration = [], config = {} } = {}) => {
    ...
    return [
      ...
      'book_appointment y request_human_booking sólo aceptan la última oferta estructurada creada por offer_appointment_slot y exigen selectionEvidence. customerQuote debe ser un mensaje completo y literal del último bloque recibido del cliente; assistantOfferQuote debe ser exactamente el localLabel de ESTE startTime y aparecer en el único mensaje de oferta inmediatamente anterior. No resumas, no uses palabras sueltas y no inventes evidencia.',
      ...
      humanBooking
        ? 'Si la persona identifica o acepta con lenguaje natural la oferta estructurada inmediatamente anterior, adjunta la evidencia literal, revalídala con request_human_booking y transfiere el caso; no pidas la misma confirmación otra vez. Nunca digas que la cita ya quedó agendada: sólo quedó solicitada al equipo.'
        : 'Si la persona identifica o acepta con lenguaje natural la oferta estructurada inmediatamente anterior, adjunta la evidencia literal, vuelve a validarla y agenda; no sustituyas el horario por otra interpretación de palabras como "tarde" o "tardecita" ni pidas la misma confirmación otra vez.',
      humanBooking
...
- Para agendar, voluntad, propuesta y confirmación son hechos distintos: "quiere ir" o proponer un horario no autoriza reservarlo. Después de consultar disponibilidad debes llamar offer_appointment_slot con UN solo startTime. Esa herramienta genera el único mensaje visible de oferta y cierra el turno: jamás escribas, reformules ni agregues una fecha u hora por tu cuenta. Sólo en el siguiente turno, si la persona confirma esa oferta estructurada, ejecuta con selectionEvidence. customerQuote es un mensaje completo del último bloque recibido, nunca un "sí" extraído; assistantOfferQuote es el localLabel exacto de ESTE slot.
