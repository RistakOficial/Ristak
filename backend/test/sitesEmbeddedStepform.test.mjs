...
test('landing form embeds render multiple form pages as an inline stepform', async () => {
  ...
  assert.match(html, /\.rstk-kind-form \.rstkSocialProfileBlock\.rstk-block-style,\.rstk-embedded-form-source-frame \.rstkSocialProfileBlock\.rstk-block-style\{justify-self:start;transform:translateX\(var\(--rstk-social-profile-nudge,clamp\(-22px,-5vw,-14px\)\)\)\}/)
  ...
  assert.match(html, /\.rstk-embedded-form-source-frame \.rstk-shell\{display:grid;grid-template-columns:minmax\(0,1fr\);width:100%;max-width:100%;min-width:0;[^}]*border-radius:var\(--rstk-page-radius,var\(--rstk-radius-lg\)\);[^}]*padding:var\(--rstk-frame-pad,var\(--rstk-pad\)\);[^}]*overflow:hidden/)
