...
test('Stripe MSI Sites runtime uses controlled split Elements and backend-filtered plans', async () => {
  ...
  assert.match(msiRuntime, /rstk-checkout-msi-toggle/)
  assert.match(msiRuntime, /rstk-checkout-msi-option/)
  assert.match(msiRuntime, /Pagar en cuotas \(meses sin intereses\)/)
