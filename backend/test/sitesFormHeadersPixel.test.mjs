...
test('Clarity bridge materializes CSS hash modals and records safe Site interactions', async () => {
  ...
  try {
    ...
    assert.match(bridgeMatch[1], /if \(status === 'disqualified'\)/)
    assert.match(bridgeMatch[1], /track\('form_disqualified', status\)/)
    assert.doesNotMatch(bridgeMatch[1], /priorityEvents = new Set\([^\n]*form_disqualified/)
