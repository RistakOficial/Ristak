...
test('el sidebar distingue intención real y no baja Sites pesado por un roce accidental', async () => {
  ...
  assert.match(sidebar, /onFocus: \(\) => scheduleSidebarRoutePrefetch\(destination\)/)
  assert.doesNotMatch(sidebar, /onPointerDown: \(\) => prefetchSidebarRoute\(destination\)/)
  assert.doesNotMatch(sidebar, /onTouchStart: \(\) => prefetchSidebarRoute\(destination\)/)
