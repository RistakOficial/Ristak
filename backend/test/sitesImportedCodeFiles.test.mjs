...
test('AI HTML editor instructions stay scoped to active code only', async () => {
  ...
  assert.match(instructions, /Diseño por defecto: si el usuario pide mostrar un perfil conectado pero NO describe otra composición visual/)
  assert.match(instructions, /mismo perfil social del editor normal/)
  assert.match(instructions, /slot nativo data-rstk-native-render="ristak" también debe conservar altura intrínseca/)
  assert.match(instructions, /No reserves una pantalla para el slot/)
  assert.match(instructions, /insignia de la red superpuesta en la esquina inferior derecha del avatar/)
  assert.match(instructions, /roseta azul exacta de cuenta verificada/)
  assert.match(instructions, /únicamente cuando el usuario pida de forma explícita otro diseño/)
...
test('external AI compatibility instructions reject forms without stable Ristak IDs', async () => {
  ...
  assert.match(videoPlayerGuide, /El slot y su padre inmediato no deben dibujar otro borde/)
  ...
  assert.match(videoGateGuide, /REQUISITO OBLIGATORIO DE ENTREGA PARA GATES/)
  ...
  assert.match(videoGateGuide, /data-rstk-video-state="playing", "paused" o "ended"/)
  assert.match(videoGateGuide, /El estado idle incluye la portada/)
  assert.match(videoGateGuide, /no uses únicamente la clase temporal rstk-video-is-playing/)
  ...
  assert.match(videoGateGuide, /Ristak debe ser la única máquina de estados/)
  assert.match(videoGateGuide, /persist, resume, seek-policy, progress-key/)
  assert.match(videoGateGuide, /PRUEBA OBLIGATORIA ANTES DE ENTREGAR/)
  assert.match(videoGateGuide, /a 390px/)
  assert.match(videoGateGuide, /data-rstk-video-gate-persist="visitor"/)
  assert.match(videoGateGuide, /data-rstk-video-gate-resume="true"/)
  assert.match(videoGateGuide, /data-rstk-video-gate-seek-policy="watched_only"/)
  assert.match(videoGateGuide, /data-rstk-video-gate-progress-key/)
  ...
  assert.match(socialProfileGuide, /fila compacta y transparente/)
  assert.match(socialProfileGuide, /roseta azul exacta de cuenta verificada/)
  assert.match(socialProfileGuide, /otro diseño o una composición diferente/)
  ...
  assert.match(builder, /El padre inmediato del slot solo puede usar max-width, margen y alineación/)
  assert.match(builder, /eso crea un segundo frame y deja franjas alrededor del video/)
  ...
  assert.match(builder, /Este es el diseño predeterminado cuando la petición no describe otro perfil/)
  assert.match(builder, /El slot debe medir solo lo que ocupa el perfil/)
  assert.match(builder, /el siguiente título, video o formulario debe empezar inmediatamente después/)
  assert.match(builder, /ChatGPT, Claude o Codex solo deben diseñar un perfil diferente/)
  assert.match(builder, /conserva exactamente la estrella de verificado, la insignia de red, la jerarquía y el acomodo del editor normal/)
  ...
  assert.match(calendarGuide, /operación real que confirma su único submit/)
  assert.match(calendarGuide, /nunca por el orden visual/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-step="id-estable"/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-kind="questions\|date\|time\|confirm\|success"/)
  assert.match(calendarGuide, /fecha → horario → preguntas → confirmar/)
  assert.match(calendarGuide, /el orden obligatorio es date → time → questions → confirm → success/)
  assert.match(calendarGuide, /No muestres situación, inversión, contacto ni ningún otro campo antes de seleccionar un horario/)
  assert.match(calendarGuide, /los datos de contacto van en el último paso questions/)
  assert.match(calendarGuide, /un solo calendario real, visible con blur pero inert/)
  assert.match(calendarGuide, /las preguntas siguen ocultas hasta que el visitante seleccione time/)
  assert.match(calendarGuide, /evento que el usuario configuró para ese elemento calendar/)
  assert.match(calendarGuide, /Schedule es solo el default recomendado/)
  assert.match(calendarGuide, /data-rstk-calendar-response="id-estable"/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-kind="questions"/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-kind="date"/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-kind="time"/)
  assert.match(calendarGuide, /data-rstk-calendar-flow-kind="confirm"/)
...
test('HTML mobile rules are shared by every creation path and the code preview uses a real phone viewport', async () => {
  ...
  assert.match(mobileGuide, /usa temporalmente el archivo horizontal de computadora como respaldo/)
  assert.match(mobileGuide, /recorte centrado 9:16/)
