...
test('la tabla consulta tracking con cursor y un contrato sin COUNT', () => {
  ...
  assert.match(serviceSource, /start: input\.start[\s\S]*end: input\.end[\s\S]*filters: input\.filters \?\? \{\}[\s\S]*q: input\.q\?\.trim\(\) \?\? ''[\s\S]*column: input\.column \|\| 'all'[\s\S]*cursor: input\.cursor \?\? null[\s\S]*limit/)
