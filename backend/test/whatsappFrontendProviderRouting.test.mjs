...
test('PhoneChat ofrece rutas explicitas de HighLevel junto a cada WhatsApp nativo', async () => {
  ...
  assert.match(source, /const preferredReplyChannel = nextChannel === 'sms_qr'[\s\S]*?nextChannel === 'whatsapp_api'/)
  assert.match(source, /updateConversationalChannelPreference\(contactId, preferredReplyChannel,[\s\S]*?routePhoneId \|\| 'highlevel'/)
...
test('DesktopChat permite elegir WhatsApp o SMS de HighLevel aunque existan numeros nativos', async () => {
  ...
  assert.match(source, /const preferredReplyChannel = !isCommentComposerChannel\(nextChannel\)/)
  assert.match(source, /updateConversationalChannelPreference\(activeContact\.id, preferredReplyChannel,[\s\S]*?nextWhatsAppRouteId \|\| null/)
