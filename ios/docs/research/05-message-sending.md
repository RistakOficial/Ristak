...
2. Al confirmar: `POST /states/:contactId {action}` para CADA agente activo del
   contacto, toast «Agente pausado 24 horas» (`"<Agente> no responderá este chat
...
Acciones por agente (con descripciones exactas):
...
11. **Multi-agente**: la confirmación pausar-y-enviar debe iterar TODOS los estados
    activos con `agentId` (un POST por agente), como /movil.
