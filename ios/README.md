...
- **Chats** (pantalla principal): bandeja con paginación/merge, búsqueda,
  ...
  **No leído → Fijar/Desfijar**, con fijados arriba y estado local persistente,
  cache en disco para arranque en frío: el inbox pinta su snapshot identificado
  de inmediato y lo revalida en silencio; su bootstrap se reinicia al resolverse
  la cuenta o el permiso de Chat, incluso sin snapshot. El hilo pide la red
  primero, esconde su snapshot 350 ms y sólo lo muestra si la respuesta viva
  tarda o falla;
  progreso real por etapas durante la
