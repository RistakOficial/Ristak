...
function getMediaAttachmentType(messageType = '', mimeType = '', name = '', mediaUrl = ''): ChatAttachmentType | null {
  ...
  if (normalizedType.includes('image') || normalizedType.includes('sticker') || imageFile) {
    return 'image'
  }
...
function getAvatarChannelBadgeClass(kind: ContactChannelBadgeKind) {
  ...
  // WhatsApp: mismo tratamiento que Messenger/Instagram — glifo blanco sobre el
  // circulo verde del badge (con su sombra y brillo), no el logo de marca suelto.
...
export const PhoneChat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      if (conversationHistoryPrependRef.current || olderMessagesLoadingRef.current) return
  ...
  useLayoutEffect(() => {
    ...
    const shouldScrollToEnd = conversationOpen && (
      ...
      (messageWasAppended && (userWasAlreadyAtBottom || lastMessage?.direction === 'outbound'))
    )
    ...
    if (conversationHistoryPrependRef.current || !shouldScrollToEnd) return
    ...
    return (
      ...
      >
        ...
        <span className={styles.messageLocationDetails}>
          ...
          {subtitle !== coordinates && <span className={styles.messageLocationCoordinates}>{coordinates}</span>}
        </span>
      </a>
    )
  }
