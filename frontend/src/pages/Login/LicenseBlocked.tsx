...
export const LicenseBlocked: React.FC = () => {
  ...
  const paymentRecovery = Boolean(state?.paymentUrl || rememberedState?.paymentUrl) && [
    'payment_failed',
    'mdp_payment_failed'
  ].includes(state?.reason || '')
  const title = trialExpired
    ? 'Tu periodo gratis terminó'
    : paymentRecovery
      ...
      <div className={styles.loginBox}>
        ...
        <div className={styles.form}>
          {paymentUrl && (
            ...
            >
              {paymentRecovery ? 'Actualizar mi tarjeta' : 'Continuar con el pago'}
            </Button>
          )}
          <Button
            type="button"
        ...
        <div className={styles.setupHint}>
          {paymentRecovery
            ? 'Tu información sigue segura. En cuanto Rebill confirme el cobro, podrás entrar de nuevo con tu misma cuenta.'
