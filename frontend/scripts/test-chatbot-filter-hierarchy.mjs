...
assert.match(phoneChatSource, /<div className=\{styles\.topActionRow\}[\s\S]{0,500}renderAgentRobotButton\([\s\S]{0,300}openAgentHubMenu/, '/movil debe conservar el acceso operativo del bot en la esquina')
assert.match(phoneChatSource, /renderAgentStatusBubble/, '/movil debe conservar el estado animado del bot')
...
assert.match(androidSource, /setAgentHubOpen\(true\)/, 'Android debe conservar el acceso operativo del bot en la esquina')
assert.match(androidSource, /<NativeAgentHubSheet[\s\S]*open=\{agentHubOpen\}/, 'Android debe abrir el Hub desde el robot')
...
assert.match(iosInboxScreenSource, /ToolbarItem\(placement: \.topBarLeading\)[\s\S]*showsAgentHub = true[\s\S]*AgentBotGlyph/, 'iOS debe conservar el acceso operativo del bot en la esquina')
assert.match(iosInboxScreenSource, /isPresented: \$showsAgentHub[\s\S]*AgentHubSheet\(\)/, 'iOS debe abrir el Hub desde el robot')
