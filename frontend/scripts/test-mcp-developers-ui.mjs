...
assert.match(settingsSource, /canonicalMcpServerUrl/)
...
assert.doesNotMatch(settingsSource, /developerInfoField[\s\S]{0,500}<button/)
...
assert.match(documentationSource, /api-access\/mcp\/status/)
assert.match(documentationSource, /canonicalMcpUrl/)
assert.doesNotMatch(documentationSource, /const mcpServerUrl = `\$\{origin\}\/api\/mcp`/)
...
assert.match(externalAccessDocs, /typed tools/)
