...
const RSTK_BASE_CSS = `
  *,*::before,*::after{box-sizing:border-box}
  ...
  .rstk-checkout-test-helper{min-width:0;border:1px solid var(--rstk-border);border-radius:var(--rstk-radius);background:color-mix(in srgb,var(--rstk-surface) 92%,transparent)}
  .rstk-checkout-test-helper summary{display:flex;align-items:center;justify-content:space-between;gap:10px;min-width:0;padding:11px 12px;color:var(--rstk-block-text,var(--rstk-ink));font-size:.86rem;font-weight:750;cursor:pointer;list-style:none}
  .rstk-checkout-test-helper summary::-webkit-details-marker{display:none}
  .rstk-checkout-test-chevron{width:8px;height:8px;border-right:2px solid currentColor;border-bottom:2px solid currentColor;transform:rotate(45deg);transition:transform .15s ease;opacity:.72}
  .rstk-checkout-test-helper[open] summary{border-bottom:1px solid var(--rstk-border)}
  .rstk-checkout-test-helper[open] .rstk-checkout-test-chevron{transform:rotate(225deg)}
  .rstk-checkout-test-body{display:grid;gap:12px;padding:12px}
  .rstk-checkout-test-body p,.rstk-checkout-test-hint{margin:0;color:var(--rstk-muted);font-size:.78rem;line-height:1.45}
  .rstk-checkout-test-hint{padding:9px 10px;border:1px solid var(--rstk-border);border-radius:var(--rstk-radius);background:var(--rstk-input-bg,var(--rstk-surface))}
  .rstk-checkout-test-section{display:grid;gap:7px;min-width:0}
  .rstk-checkout-test-section>strong{color:var(--rstk-block-text,var(--rstk-ink));font-size:.78rem;font-weight:800}
  .rstk-checkout-test-scroll{min-width:0;overflow-x:auto;border:1px solid var(--rstk-border);border-radius:var(--rstk-radius)}
  .rstk-checkout-test-table{width:100%;min-width:620px;border-collapse:collapse;background:var(--rstk-input-bg,var(--rstk-surface))}
  .rstk-checkout-test-table th,.rstk-checkout-test-table td{padding:8px 9px;border-top:1px solid var(--rstk-border);color:var(--rstk-muted);font-size:.76rem;line-height:1.35;text-align:left;white-space:nowrap}
  .rstk-checkout-test-table th{border-top:0;color:var(--rstk-muted);font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.05em;background:color-mix(in srgb,var(--rstk-ink) 5%,transparent)}
  .rstk-checkout-test-table td:last-child{white-space:normal}
  .rstk-checkout-test-copy-value{display:inline-flex;align-items:center;gap:6px}
  .rstk-checkout-test-copy-value>span{font-variant-numeric:tabular-nums}
  .rstk-checkout-test-copy{min-height:24px;padding:0 7px;border:1px solid var(--rstk-border);border-radius:999px;background:var(--rstk-input-bg,var(--rstk-surface));color:var(--rstk-block-text,var(--rstk-ink));font:inherit;font-size:.68rem;font-weight:800;cursor:pointer}
  .rstk-checkout-test-copy:hover{border-color:var(--rstk-accent);color:var(--rstk-accent)}
  ...
  @media (max-width:560px){.rstk-checkout-test-table{min-width:560px}.rstk-checkout-test-helper summary{align-items:flex-start}}
  ...
  @media (max-width:760px){.rstk-video.rstk-video-landscape.rstk-video-mobile-portrait-crop:not(.rstk-video-form-gate-fit-expanded){aspect-ratio:9/16}.rstk-video.rstk-video-landscape.rstk-video-mobile-portrait-crop:not(.rstk-video-form-gate-fit-expanded) > video{object-fit:cover!important}}
	  ...
	  .rstk-video-control-button:focus-visible{outline:2px solid currentColor;outline-offset:2px}
	  ...
	  .rstk-video-speed-control{flex:0 0 auto;position:relative;min-width:52px;height:32px;display:inline-flex;align-items:center;border-radius:999px;background:color-mix(in srgb,currentColor 12%,transparent);color:inherit;padding:0 20px 0 10px}
	  ...
	  .rstk-video-speed-control option{background:Canvas;color:CanvasText}
	  .rstk-video-settings-control{position:relative;display:grid;place-items:center;flex:0 0 auto}
	  .rstk-video-settings-toggle[aria-expanded="true"]{background:color-mix(in srgb,currentColor 24%,transparent)}
	  .rstk-video-settings-toggle svg{display:block;width:16px;height:16px}
	  .rstk-video-settings-menu{position:absolute;right:0;bottom:calc(100% + 10px);z-index:8;box-sizing:border-box;display:grid;width:152px;max-width:calc(100cqw - 20px);gap:7px;border:1px solid color-mix(in srgb,currentColor 18%,transparent);border-radius:14px;background:var(--rstk-video-player-color,#000000);color:var(--rstk-video-play-color,#fff);box-shadow:0 20px 48px -24px color-mix(in srgb,var(--rstk-video-player-color,currentColor) 90%,transparent);padding:9px;backdrop-filter:blur(22px) saturate(140%);-webkit-backdrop-filter:blur(22px) saturate(140%)}
	  .rstk-video-settings-menu[hidden]{display:none!important}
	  .rstk-video-settings-title{display:block;padding:2px 5px 4px;color:inherit;font-size:.7rem;font-weight:650;letter-spacing:.02em;opacity:.72}
	  .rstk-video-settings-options{display:grid;gap:3px}
	  .rstk-video-settings-option{box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;min-height:34px;border:0;border-radius:9px;background:transparent;color:inherit;cursor:pointer;font:inherit;font-size:.76rem;font-weight:550;padding:7px 8px;text-align:left}
	  .rstk-video-settings-option:hover,.rstk-video-settings-option-active{background:color-mix(in srgb,currentColor 16%,transparent)}
	  .rstk-video-settings-option:focus-visible{outline:2px solid currentColor;outline-offset:-1px}
	  ...
	  .rstk-video-form-field[data-invalid] input,.rstk-video-form-field[data-invalid] textarea,.rstk-video-form-field[data-invalid] select,.rstk-video-form-field[data-invalid] .rstk-option,.rstk-video-form-field[data-invalid] .rstk-multiselect > summary{border-color:var(--rstk-form-error,var(--neg,var(--rstk-accent)))!important;box-shadow:0 0 0 3px color-mix(in srgb,var(--rstk-form-error,var(--neg,var(--rstk-accent))) 16%,transparent)}
  ...
  input:not([type='radio']):not([type='checkbox']):focus,textarea:focus,select:focus{box-shadow:0 0 0 3px color-mix(in srgb,var(--rstk-muted) 20%,transparent)}
	  ...
	  .rstk-kind-form.rstk-select-filled .rstk-field select,.rstk-kind-form.rstk-select-filled .rstk-field .rstk-multiselect > summary,.rstk-select-filled .rstk-embedded-form select,.rstk-select-filled .rstk-embedded-form .rstk-multiselect > summary,.rstk-select-filled .rstk-video-form-field select,.rstk-select-filled .rstk-video-form-field .rstk-multiselect > summary{background-color:color-mix(in srgb,var(--rstk-form-field-bg,transparent) 88%,var(--rstk-accent) 12%)}
	  .rstk-kind-form.rstk-select-underline .rstk-field select,.rstk-kind-form.rstk-select-underline .rstk-field .rstk-multiselect > summary,.rstk-select-underline .rstk-embedded-form select,.rstk-select-underline .rstk-embedded-form .rstk-multiselect > summary,.rstk-select-underline .rstk-video-form-field select,.rstk-select-underline .rstk-video-form-field .rstk-multiselect > summary{border-width:0 0 var(--rstk-form-field-border-width,1px);border-radius:0;background-color:transparent;padding-left:0;padding-right:36px}
	  .rstk-kind-form.rstk-select-soft .rstk-field select,.rstk-kind-form.rstk-select-soft .rstk-field .rstk-multiselect > summary,.rstk-select-soft .rstk-embedded-form select,.rstk-select-soft .rstk-embedded-form .rstk-multiselect > summary,.rstk-select-soft .rstk-video-form-field select,.rstk-select-soft .rstk-video-form-field .rstk-multiselect > summary{border-radius:999px;background-color:color-mix(in srgb,var(--rstk-form-field-bg,var(--rstk-input-bg)) 90%,var(--rstk-accent) 10%);padding-left:20px}
	  ...
	  .rstk-multiselect{position:relative;width:100%;min-width:0;color:var(--rstk-form-field-text,var(--rstk-input-ink))}
	  .rstk-multiselect > summary{box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;min-height:var(--rstk-form-field-height,50px);border:var(--rstk-form-field-border-width,1px) solid var(--rstk-form-field-border,var(--rstk-input-border));border-radius:var(--rstk-form-field-radius,var(--rstk-field-radius,var(--rstk-radius)));background:var(--rstk-form-field-bg,var(--rstk-input-bg));color:var(--rstk-form-field-text,var(--rstk-input-ink));font-family:var(--rstk-form-font,var(--rstk-font));font-size:var(--rstk-form-input-size,1rem);font-style:var(--rstk-form-font-style,normal);font-weight:var(--rstk-form-weight,500);text-decoration:var(--rstk-form-text-decoration,none);padding:var(--rstk-form-field-pad-y,13px) var(--rstk-form-field-pad-x,14px);cursor:pointer;list-style:none}
	  .rstk-multiselect > summary::-webkit-details-marker{display:none}
	  .rstk-multiselect > summary::after{content:'';flex:0 0 auto;width:7px;height:7px;margin-left:12px;border-right:2px solid currentColor;border-bottom:2px solid currentColor;opacity:.62;transform:translateY(-2px) rotate(45deg);transition:transform .15s ease}
	  .rstk-multiselect[open] > summary{border-color:var(--rstk-accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--rstk-accent) 14%,transparent)}
	  .rstk-multiselect[open] > summary::after{transform:translateY(2px) rotate(225deg)}
	  .rstk-multiselect-menu{position:absolute;z-index:20;top:calc(100% + 6px);left:0;right:0;display:grid;gap:4px;max-height:260px;overflow:auto;border:var(--rstk-form-field-border-width,1px) solid var(--rstk-form-field-border,var(--rstk-input-border));border-radius:var(--rstk-form-field-radius,var(--rstk-field-radius,var(--rstk-radius)));background:var(--rstk-form-field-bg,var(--rstk-input-bg));box-shadow:0 18px 38px -24px color-mix(in srgb,var(--rstk-form-field-text,var(--rstk-input-ink)) 48%,transparent);padding:6px}
	  .rstk-video-form-gate .rstk-multiselect-menu{position:static;margin-top:6px}
	  .rstk-multiselect-menu .rstk-option{min-height:40px;border:0;border-radius:calc(var(--rstk-form-field-radius,var(--rstk-field-radius,var(--rstk-radius))) * .7);background:transparent;padding:9px 10px}
	  .rstk-multiselect-menu .rstk-option:hover,.rstk-multiselect-menu .rstk-option:has(input:checked){background:var(--rstk-form-choice-selected-bg,color-mix(in srgb,var(--rstk-accent) 8%,transparent))}
  ...
  .rstk-field.rstk-fieldstyled select,.rstk-field.rstk-fieldstyled .rstk-multiselect > summary{border-width:var(--rstk-form-field-border-width,1px);border-radius:var(--rstk-form-field-radius,var(--rstk-field-radius,var(--rstk-radius)));background-color:var(--rstk-form-field-bg,var(--rstk-input-bg));padding-left:9px;padding-right:42px}
  ...
  .rstk-field.rstk-select-filled select,.rstk-field.rstk-select-filled .rstk-multiselect > summary{background-color:color-mix(in srgb,var(--rstk-form-field-bg,transparent) 88%,var(--rstk-accent) 12%)}
  .rstk-field.rstk-select-underline select,.rstk-field.rstk-select-underline .rstk-multiselect > summary{border-width:0 0 var(--rstk-form-field-border-width,1px);border-radius:0;background-color:transparent;padding-left:0;padding-right:36px}
  .rstk-field.rstk-select-soft select,.rstk-field.rstk-select-soft .rstk-multiselect > summary{border-radius:999px;background-color:color-mix(in srgb,var(--rstk-form-field-bg,var(--rstk-input-bg)) 90%,var(--rstk-accent) 10%);padding-left:20px}
  ...
  .rstk-field[data-invalid] > input,.rstk-field[data-invalid] > textarea,.rstk-field[data-invalid] > select,.rstk-field[data-invalid] .rstk-phone-input > input,.rstk-field[data-invalid] .rstk-phone-input > select,.rstk-field[data-invalid] .rstk-option,.rstk-field[data-invalid] .rstk-multiselect > summary{border-color:var(--rstk-form-error,var(--neg,var(--rstk-accent)))!important;box-shadow:0 0 0 3px color-mix(in srgb,var(--rstk-form-error,var(--neg,var(--rstk-accent))) 14%,transparent)}
  ...
  .rstk-social-platform-facebook{background:#1877f2 url("data:image/webp;base64,") center/100% 100% no-repeat}
  .rstk-social-platform-instagram{background:#fff url("data:image/webp;base64,") center/100% 100% no-repeat}
...
function isVideoMobilePortraitCropEnabled(settings = {}) {
  return settings?.videoMobilePortraitCrop !== false
}
