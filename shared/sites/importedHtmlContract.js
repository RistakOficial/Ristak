...
export const IMPORTED_HTML_VIDEO_GATE_RULES = Object.freeze([
  ...
  'REQUISITO OBLIGATORIO DE ENTREGA PARA GATES: no termines ni respondas status="ready" hasta comprobar que cada ID tenga al menos un slot fuente con trigger y value, un data-rstk-video-gate-shell, un estado visible data-rstk-video-gate-locked y TODO el contenido real dentro de data-rstk-video-gate-content. Todos esos hooks deben repetir exactamente el mismo ID no vacío; un gate sin cualquiera de esas piezas está incompleto aunque el CSS parezca ocultar la sección.',
  ...
  'Ristak debe ser la única máquina de estados del gate. En el HTML fuente no agregues hidden, inert, aria-hidden ni display:none como condición manual de data-rstk-video-gate-content, ni selectores :has(...) que decidan por su cuenta cuándo mostrarlo. El renderer aplica el estado inicial fail-closed y publica data-rstk-video-gate-state="locked|unlocked"; duplicar esa lógica puede dejar un atributo residual ocultando el contenido aun después de cumplir el umbral.',
  ...
  'Para computadora y móvil con videos distintos, ambos slots usan el mismo data-rstk-video-gate-id, trigger y value. También deben coincidir persist, resume, seek-policy, progress-key y la vigencia progress-days/progress-ttl. Ristak toma el mayor progreso real de las variantes y nunca suma dos reproducciones simultáneas; una sola diferencia deja dos fuentes de verdad y puede impedir el desbloqueo o la reanudación.',
  ...
  'PRUEBA OBLIGATORIA ANTES DE ENTREGAR: usa una sesión limpia, confirma que el contenido inicia bloqueado, pulsa play y reproduce contenido real hasta rebasar el umbral; entonces data-rstk-video-gate-locked debe desaparecer y data-rstk-video-gate-content quedar visible e interactivo. Recarga y vuelve a entrar desde el mismo navegador: con persist="visitor" debe conservar el avance o permanecer desbloqueado. Repite la prueba en escritorio y a 390px; si el Site ya está publicado o la publicación fue autorizada, valida también la URL pública real porque una captura estática o revisar solamente el HTML no demuestra el runtime.'
])
