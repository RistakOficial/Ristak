// Shared by the contact list and notification filters. Keep one field/operator catalog.
const textOperators                          = [
...
const automationWaitKindOptions                          = [
...
export const CONTACT_ADVANCED_FIELD_GROUPS                              = [
  {
    label: 'Contacto',
    fields: [
...
export const getContactAdvancedOperators = (field                       )                          => {
  if (!field) return textOperators
  if (field.key === 'priority') return selectOperators.filter(option => ['is', 'is_not'].includes(option.value))
